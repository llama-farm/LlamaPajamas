"""Test GGUF runtime with API server."""
...
def wait_for_server(host="127.0.0.1", port=8002, timeout=90):
...
def test_chat_completion(host="127.0.0.1", port=8002):
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": "What is JavaScript in one sentence?"},
        ],
        "max_tokens": 50,
        "temperature": 0.7,
        "stream": False,
...
def test_streaming_chat(host="127.0.0.1", port=8002):
...
def test_list_models(host="127.0.0.1", port=8002):
...
def main():
    """Run all tests."""
    print("\n" + "=" * 70)
    print("Llama-Pajamas GGUF Runtime Test Suite")
    print("=" * 70)

    # Find GGUF file
    gguf_files = glob.glob(str(Path(__file__).parent.parent / "quant" / "models" / "qwen3-8b" / "gguf" / "*.gguf"))
    if not gguf_files:
        print("\n❌ No GGUF files found")
        sys.exit(1)
    ...
    # cd run-gguf
    # uv run llama-pajamas-gguf --model-path <path> --port 8002
    ...
    if not wait_for_server(host, port, timeout=90):
        ...
        print(f"   cd run-gguf")
        print(f"   uv run llama-pajamas-gguf --model-path {model_path} --port {port}")
        sys.exit(1)
